...
def prepare_spkfiles(data_root_dir,
                     spk_save_dir,
                     dict_save_dir):
    if not os.path.isdir(spk_save_dir) or len(os.listdir(spk_save_dir)) == 0:
        spk_dict = dict()
        ...
        name_to_id = dict()
        for i, key in enumerate(spk_dict.keys()):
        ...
        np.save(os.path.join(dict_save_dir, r"spk_dict.npy"), name_to_id)
